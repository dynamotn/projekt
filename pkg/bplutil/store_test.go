...
func TestValidate_SaysWhatASourceCanBe(t *testing.T) {
	recipes, _ := useStores(t)
	// Anything that is not one of the two origins leaves the source empty.
	...
	if !strings.Contains(err.Error(), "source.template") || !strings.Contains(err.Error(), "source.repo") {
		t.Errorf("Get() error = %v, want it to name both origins", err)
	}
}
