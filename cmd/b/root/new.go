...
func askForValues(cmd *cobra.Command, o bplutil.CreateOptions) (tplutil.Values, error) {
	...
	delims, err := tplutil.Delimiters(plan.Template)
	if err != nil {
		return nil, err
	}

	values, err := tplutil.Prompter{In: cmd.InOrStdin(), Out: prompt, Delims: delims, Origin: bplutil.TrustOrigin(o.Recipe)}.Ask(vars, o.Values, base)
	if err != nil {
		return nil, err
	}
	fmt.Fprintln(prompt)
