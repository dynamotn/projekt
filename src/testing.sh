...
# @env DYBATPHO_TEST_UPDATE_SNAPSHOTS string Set to `true`, `yes`, or `on` to rewrite snapshots instead of comparing them
...
# @env DYBATPHO_TEST_UPDATE_SNAPSHOTS string Set to a truthy value to rewrite the snapshot
...
function dybatpho::assert_snapshot {
  ...
  if [[ ! -f "${snapshot_file}" ]] || dybatpho::is true "${DYBATPHO_TEST_UPDATE_SNAPSHOTS}"; then
